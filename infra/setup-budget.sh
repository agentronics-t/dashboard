...
#
# Usage: ./infra/setup-budget.sh <BILLING_ACCOUNT_ID> <PROJECT_ID> <AMOUNT_USD>
#   e.g. ./infra/setup-budget.sh 01FE97-2E43EB-0A1E34 project-6d96c09a-5821-4133-959 50

set -euo pipefail

BILLING_ACCOUNT="${1:?Usage: setup-budget.sh <BILLING_ACCOUNT_ID> <PROJECT_ID> <AMOUNT_USD>}"
PROJECT_ID="${2:?missing PROJECT_ID}"
AMOUNT="${3:?missing AMOUNT_USD}"
...
if [[ -n "${EXISTING}" ]]; then
  echo "▶ Updating budget ${NAME} → ${AMOUNT} USD/mo"
  gcloud billing budgets update "${EXISTING}" "${ARGS[@]:2}"
else
  echo "▶ Creating budget ${NAME} → ${AMOUNT} USD/mo (alerts at 50/90/100%)"
